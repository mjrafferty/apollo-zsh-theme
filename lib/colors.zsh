...
getColorCode() {
  if (( ARGC == 1 )); then
    case $1 in
      foreground)
        local k
        for k in "${(k@)__APOLLO_COLORS}"; do
          local v=${__APOLLO_COLORS[$k]}
          print -P "%F{$v}$v - $k%f"
        done
        return
        ;;
...
_apollo_translate_color() {
  ...
  if [[ $1 == '<->' ]]; then     # decimal color code: 255
    ...
    _APOLLO_RETURN_MESSAGE=$__APOLLO_COLORS[${${${1#bg-}#fg-}#br}]
  fi

}

# Resolves a color to its numerical value, or an empty string. Communicates the result back
# by setting _APOLLO_RETURN_MESSAGE.
_apollo_color() {

  local user_var=APOLLO_${(U)${2}#prompt_}_${3}

  _apollo_translate_color ${${(P)user_var}:-${1}}
...
_apollo_background() {
  ...
  if [[ -n $1 ]]; then
    _APOLLO_RETURN_MESSAGE="%K{$1}"
  else
    _APOLLO_RETURN_MESSAGE="%k"
  fi
...
_apollo_foreground() {
  ...
  if [[ -n $1 ]]; then
    _APOLLO_RETURN_MESSAGE="%F{$1}";
  else
    _APOLLO_RETURN_MESSAGE="%f"
  fi
