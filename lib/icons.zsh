...
# icon-functions for the apollo-ZSH-theme
# https://github.com/bhilburn/apollo
################################################################

# These characters require the Powerline fonts to work properly. If you see
# boxes or bizarre characters below, your fonts are not correctly installed. If
# you do not want to install a special font, you can set `APOLLO_MODE` to
# `compatible`. This shows all icons in regular symbols.

# Initialize the icon list according to the user's `APOLLO_MODE`.
typeset -gAH icons


typeset -gi _APOLLO_ICONS_INITIALIZED=0

_apollo_init_icons() {
  (( _APOLLO_ICONS_INITIALIZED )) && return
  _APOLLO_ICONS_INITIALIZED=1
  ...
  if [[ "$APOLLO_HIDE_BRANCH_ICON" == true ]]; then
      icons[VCS_BRANCH_ICON]=''
  fi
}
...
print_icon() {
  ...
  local icon_name=$1
  local var_name=APOLLO_${icon_name}
...
# Sets _APOLLO_RETURN_MESSAGE to the icon whose name is supplied via $1.
_apollo_get_icon() {

  local var_name=APOLLO_$1

  _APOLLO_RETURN_MESSAGE=${${${(P)var_name}-$icons[$1]}}

  [[ $_APOLLO_RETURN_MESSAGE != $'\b'? ]] || _APOLLO_RETURN_MESSAGE="%{$_APOLLO_RETURN_MESSAGE%}"  # penance for past sins
