...
+vi-git-untracked() {
  ...
  if [[ -z $untrackedFiles && "$APOLLO_VCS_SHOW_SUBMODULE_DIRTY" == "true" ]]; then
...
+vi-git-remotebranch() {
    ...
    if [[ -n "$APOLLO_VCS_SHORTEN_LENGTH" ]]; then
     if [ ${#hook_com[branch]} -gt ${APOLLO_VCS_SHORTEN_LENGTH} ]; then
       case "$APOLLO_VCS_SHORTEN_STRATEGY" in
         truncate_middle)
           hook_com[branch]="${branch_name:0:$APOLLO_VCS_SHORTEN_LENGTH}${APOLLO_VCS_SHORTEN_DELIMITER}${branch_name: -$APOLLO_VCS_SHORTEN_LENGTH}"
         ;;
         truncate_from_right)
           hook_com[branch]="${branch_name:0:$APOLLO_VCS_SHORTEN_LENGTH}${APOLLO_VCS_SHORTEN_DELIMITER}"
         ;;
       esac
     fi
    fi
...
+vi-git-tagname() {
    if [[ "$APOLLO_VCS_HIDE_TAGS" == "false" ]]; then
        ...
        if [[ -n "${tag}" ]] ; then
            ...
            if [[ -z $(command git symbolic-ref HEAD 2>/dev/null) ]]; then
                ...
                # string.
                local revision
                revision=$(command git rev-list -n 1 --abbrev-commit --abbrev=${APOLLO_VCS_INTERNAL_HASH_LENGTH} HEAD)
